 ...
 */
public class ThemedActivity extends android.app.Fragment {
...
//  }
    ...
    public void setNavBarColor() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            if (isNavigationBarColored()) getActivity().getWindow().setNavigationBarColor(getPrimaryColor());
            else
                getActivity().getWindow().setNavigationBarColor(ContextCompat.getColor(getActivity(), R.color.md_black_1000));
        }
    }
    ...
    protected void setStatusBarColor() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            if (isTranslucentStatusBar())
                getActivity().getWindow().setStatusBarColor(ColorPalette.getObscuredColor(getPrimaryColor()));
            else
                getActivity().getWindow().setStatusBarColor(getPrimaryColor());
        }
    }
    ...
    public void setRecentApp(String text){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            BitmapDrawable drawable = ((BitmapDrawable) getActivity().getDrawable(R.mipmap.ic_launcher));
