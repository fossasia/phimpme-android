...
public class FlickrActivity extends ThemedActivity {
  ...
  public void onOAuthDone(OAuth result) {
    if (result == null) {
      SnackBarHandler.create(parent, getString(R.string.auth_failed)).show();
    } else {
  ...
  /**
   * This method is called two times during the authentication. In the first time, only tokenSecret
   * will have some value and others will be null and in the second time this method will called
   * with all the details of the account. So after the two calls of this function the account abject
   * will be fully populated in realm database.
   */
  public void saveOAuthToken(String userName, String userId, String token, String tokenSecret) {
    ...
    if (result.size() == 0) account = realm.createObject(AccountDatabase.class, FLICKR.toString());
    else account = result.first();
  ...
  public class OAuthTask extends AsyncTask<Void, Integer, String> {
    ...
    private final Uri OAUTH_CALLBACK_URI =
        Uri.parse(FlickrActivity.CALLBACK_SCHEME + "://oauth"); // $NON-NLS-1$
    ...
    protected void onPreExecute() {
      ...
      dialog =
          AlertDialogsHelper.getProgressDialog(
              FlickrActivity.this,
              progressDialog,
              getContext().getString(R.string.authenticating_your_app_message),
    ...
    protected void onPostExecute(String result) {
      ...
      if (result != null && !result.startsWith("error")) {
        ...
        SnackBarHandler.create(parent, result).show();
      }
    }
  }
}
