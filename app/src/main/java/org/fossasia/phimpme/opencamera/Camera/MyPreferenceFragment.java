...
/**
 * Fragment to handle the Settings UI. Note that originally this was a PreferenceActivity rather
 * than a PreferenceFragment which required all communication to be via the bundle (since this
 * replaced the CameraActivity, meaning we couldn't access data from that class. This no longer
 * applies due to now using a PreferenceFragment, but I've still kept with transferring information
 * via the bundle (for the most part, at least).
 */
public class MyPreferenceFragment extends PreferenceFragment
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    if (MyDebug.LOG) Log.d(TAG, "nCameras: " + nCameras);
    ...
    // readFromBundle(bundle, "color_effects", Preview.getColorEffectPreferenceKey(),
    // Camera.Parameters.EFFECT_NONE, "preference_category_camera_effects");
    // readFromBundle(bundle, "scene_modes", Preview.getSceneModePreferenceKey(),
    // Camera.Parameters.SCENE_MODE_AUTO, "preference_category_camera_effects");
    // readFromBundle(bundle, "white_balances", Preview.getWhiteBalancePreferenceKey(),
    ...
    if (MyDebug.LOG) Log.d(TAG, "supports_face_detection: " + supports_face_detection);

    if (!supports_face_detection) {
      Preference pref = findPreference("preference_face_detection");
      PreferenceGroup pg =
          (PreferenceGroup) this.findPreference("preference_category_camera_controls");
      pg.removePreference(pref);
    }
    ...
    if (widths != null && heights != null) {
      ...
      for (int i = 0; i < widths.size(); i++) {
        entries[i] =
            widths.get(i)
                ...
                + heights.get(i)
                + " "
                + Preview.getAspectRatioMPString(widths.get(i), heights.get(i));
      ...
      PreferenceGroup pg =
          (PreferenceGroup) this.findPreference("preference_screen_photo_settings");
      pg.removePreference(pref);
    }
          ...
          new OnPreferenceChangeListener() {
            ...
            public boolean onPreferenceChange(Preference preference, Object newValue) {
              ...
              if (newValue.equals("preference_raw_yes")) {
                // we check done_raw_info every time, so that this works if the user selects RAW
                // again without leaving and returning to Settings
                boolean done_raw_info =
                    sharedPreferences.contains(PreferenceKeys.getRawInfoPreferenceKey());
                if (!done_raw_info) {
    ...
    if (MyDebug.LOG) Log.d(TAG, "max_expo_bracketing_n_images: " + max_expo_bracketing_n_images);

    final boolean supports_exposure_compensation =
    ...
    if (MyDebug.LOG) Log.d(TAG, "supports_iso_range: " + supports_iso_range);

    final boolean supports_exposure_time = bundle.getBoolean("supports_exposure_time");
    if (MyDebug.LOG) Log.d(TAG, "supports_exposure_time: " + supports_exposure_time);

    final boolean supports_white_balance_temperature =
    ...
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR1 || !can_disable_shutter_sound) {
      ...
      PreferenceGroup pg =
          (PreferenceGroup) this.findPreference("preference_screen_camera_controls_more");
      pg.removePreference(pref);
    }
    ...
    if (!using_android_l) {
      ...
      PreferenceGroup pg =
          (PreferenceGroup) this.findPreference("preference_category_photo_debugging");
      pg.removePreference(pref);
          ...
          new OnPreferenceClickListener() {
            ...
            public boolean onPreferenceClick(Preference arg0) {
              if (pref.getKey().equals("preference_use_camera2")) {
                if (MyDebug.LOG) Log.d(TAG, "user clicked camera2 API - need to restart");
                // see
            ...
            public boolean onPreferenceClick(Preference arg0) {
              if (pref.getKey().equals("preference_using_saf")) {
                ...
                if (sharedPreferences.getBoolean(
                  ...
                  // seems better to alway re-show the dialog when the user selects, to make it
                  // clear where files will be saved (as the SAF location in general will be
                  // different to the non-SAF one)
                  // String uri =
  ...
  public void onResume() {
    ...
    // note, setting color here only seems to affect the "main" preference fragment screen, and not
    // sub-screens
    // note, on Galaxy Nexus Android 4.3 this sets to black rather than the dark grey that the
    // background theme should be (and what the sub-screens use); works okay on Nexus 7 Android 5
    // we used to use a light theme for the PreferenceFragment, but mixing themes in same activity
    // seems to cause problems (e.g., for EditTextPreference colors)
    TypedArray array =
        getActivity().getTheme().obtainStyledAttributes(new int[] {android.R.attr.colorBackground});
    ...
    if (getView() != null) getView().setBackgroundColor(backgroundColor);
    array.recycle();
