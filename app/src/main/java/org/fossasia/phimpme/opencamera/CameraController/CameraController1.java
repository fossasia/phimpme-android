...
public class CameraController1 extends CameraController {
  ...
  private static final int max_expo_bracketing_n_images =
      3; // seem to have problems with 5 images in some cases, e.g., images coming out same
  // brightness on OnePlus 3T
   ...
   *     CameraControllerManager1.getNumberOfCameras()-1).
   * @param camera_error_cb onError() will be called if the camera closes due to serious error. No
   *     more calls to the CameraController1 object should be made (though a new one can be created,
   *     to try reopening the camera).
   * @throws CameraControllerException if the camera device fails to open.
   */
  public CameraController1(int cameraId, final ErrorCallback camera_error_cb)
      throws CameraControllerException {
    super(cameraId);
    if (MyDebug.LOG) Log.d(TAG, "create new CameraController1: " + cameraId);
    this.camera_error_cb = camera_error_cb;
    try {
      camera = Camera.open(cameraId);
    } catch (RuntimeException e) {
      if (MyDebug.LOG) Log.e(TAG, "failed to open camera");
      e.printStackTrace();
      throw new CameraControllerException();
    }
    if (camera == null) {
      // Although the documentation says Camera.open() should throw a RuntimeException, it seems
      // that it some cases it can return null
  ...
  private List<String> convertFlashModesToValues(List<String> supported_flash_modes) {
    ...
    // Samsung Galaxy S7 at least for front camera has supported_flash_modes: auto, beach,
    // portrait?!
    // so rather than checking supported_flash_modes, we should check output_modes here
    // this is always why we check whether the size is greater than 1, rather than 0 (this also
    // matches
    ...
    if (output_modes.size() > 1) {
      ...
      if (isFrontFacing()) {
        ...
        output_modes
            .clear(); // clear any pre-existing mode (see note above about Samsung Galaxy S7)
        output_modes.add("flash_off");
        output_modes.add("flash_frontscreen_on");
      } else {
        if (MyDebug.LOG) Log.d(TAG, "no flash");
        // probably best to not return any modes, rather than one mode (see note about about Samsung
        // Galaxy S7)
        output_modes.clear();
      }
    }
  ...
  public CameraFeatures getCameraFeatures() {
    ...
    if (camera_features.is_zoom_supported) {
      ...
      try {
        ...
        // crash java.lang.NumberFormatException: Invalid int: " 500" reported in v1.4 on device
        // "es209ra", Android 4.1, 3 Jan 2014
        // this is from java.lang.Integer.invalidInt(Integer.java:138) - unclear if this is a bug in
        // Open Camera, all we can do for now is catch it
        if (MyDebug.LOG) Log.e(TAG, "NumberFormatException in getZoomRatios()");
        e.printStackTrace();
    ...
    // camera_features.supported_flash_modes = parameters.getSupportedFlashModes(); // Android
    // format
    ...
    if (camera_video_sizes == null) {
      // if null, we should use the preview sizes - see
    ...
    // Determine view angles. Note that these can vary based on the resolution - and since we read
    // these before the caller has
    // set the desired resolution, this isn't strictly correct. However these are presumably view
    // angles for the photo anyway,
    // when some callers (e.g., DrawPreview) want view angles for the preview anyway - so these will
    // only be an approximation for
    // what we want anyway.
  ...
  public SupportedValues setISO(String value) {
    ...
    if (iso_values != null && iso_values.length() > 0) {
      if (MyDebug.LOG) Log.d(TAG, "iso_values: " + iso_values);
    ...
    if (parameters.get(iso_key) == null) {
      ...
      if (parameters.get(iso_key) == null) {
        ...
        if (parameters.get(iso_key) == null) {
          if (Build.MODEL.contains("Z00")) iso_key = "iso"; // Asus Zenfone 2 Z00A and Z008: see
          // https://sourceforge.net/p/opencamera/tickets/183/
    ...
    if (iso_key != null) {
      if (values == null) {
        // set a default for some devices which have an iso_key, but don't give a list of supported
        // ISOs
        values = new ArrayList<>();
        values.add("auto");
        values.add("50");
      ...
      if (supported_values != null) {
        if (MyDebug.LOG) Log.d(TAG, "set: " + iso_key + " to: " + supported_values.selected_value);
  ...
  public void setPreviewFpsRange(int min, int max) {
    if (MyDebug.LOG) Log.d(TAG, "setPreviewFpsRange: " + min + " to " + max);
    Camera.Parameters parameters = this.getParameters();
  ...
  public void setFlashValue(String flash_value) {
    ...
    if (flash_value.equals("flash_frontscreen_on")) {
      // we do this check first due to weird behaviour on Samsung Galaxy S7 front camera where
      // parameters.getFlashMode() returns values (auto, beach, portrait)
    ...
    if (flash_mode.length() > 0 && !flash_mode.equals(parameters.getFlashMode())) {
      if (parameters.getFlashMode().equals(Camera.Parameters.FLASH_MODE_TORCH)
        ...
        // workaround for bug on Nexus 5 and Nexus 6 where torch doesn't switch off until we set
        // FLASH_MODE_OFF
        ...
        handler.postDelayed(
            new Runnable() {
              @Override
              public void run() {
                if (MyDebug.LOG) Log.d(TAG, "now set actual flash mode after turning torch off");
                if (camera
                    != null) { // make sure camera wasn't released in the meantime (has a Google
                  // Play crash as a result of this)
  ...
  private String convertFlashModeToValue(String flash_mode) {
    ...
    if (MyDebug.LOG) Log.d(TAG, "convertFlashModeToValue: " + flash_mode);
    String flash_value = "";
    if (flash_mode == null) {
      // ignore, leave focus_value at ""
  ...
  public void setRecordingHint(boolean hint) {
    if (MyDebug.LOG) Log.d(TAG, "setRecordingHint: " + hint);
    Camera.Parameters parameters = this.getParameters();
    // Calling setParameters here with continuous video focus mode causes preview to not restart
    // after taking a photo on Galaxy Nexus?! (fine on my Nexus 7).
    // The issue seems to specifically be with setParameters (i.e., the problem occurs even if we
    // don't setRecordingHint).
    // In addition, I had a report of a bug on HTC Desire X, Android 4.0.4 where the saved video was
    // corrupted.
    // This worked fine in 1.7, then not in 1.8 and 1.9, then was fixed again in 1.10
    // The only thing in common to 1.7->1.8 and 1.9-1.10, that seems relevant, was adding this code
    // to setRecordingHint() and setParameters() (unclear which would have been the problem),
    // so we should be very careful about enabling this code again!
    // Update for v1.23: the bug with Galaxy Nexus has come back (see comments in
    // Preview.setPreviewFps()) and is now unavoidable,
    // but I've still kept this check here - if nothing else, because it apparently caused video
    // recording problems on other devices too.
    // Update for v1.29: this doesn't seem to happen on Galaxy Nexus with continuous picture focus
    // mode, which is what we now use; but again, still keepin the check here due to possible
  ...
  public void setLocationInfo(Location location) {
    ...
    parameters.setGpsTimestamp(
        System.currentTimeMillis() / 1000); // initialise to a value (from Android camera source)
    parameters.setGpsLatitude(location.getLatitude());
    parameters.setGpsLongitude(location.getLongitude());
    parameters.setGpsProcessingMethod(
        location
            .getProvider()); // from http://boundarydevices.com/how-to-write-an-android-camera-app/
  ...
  public boolean supportsAutoFocus() {
    ...
    String focus_mode = parameters.getFocusMode();
    // getFocusMode() is documented as never returning null, however I've had null pointer
    // exceptions reported in Google Play from the below line (v1.7),
    // on Galaxy Tab 10.1 (GT-P7500), Android 4.0.3 - 4.0.4; HTC EVO 3D X515m (shooteru), Android
    // 4.0.3 - 4.0.4
    if (focus_mode != null
        && (focus_mode.equals(Camera.Parameters.FOCUS_MODE_AUTO)
            || focus_mode.equals(Camera.Parameters.FOCUS_MODE_MACRO))) {
  ...
  public void autoFocus(
      final CameraController.AutoFocusCallback cb, boolean capture_follows_autofocus_hint) {
    if (MyDebug.LOG) Log.d(TAG, "autoFocus");
    Camera.AutoFocusCallback camera_cb =
        new Camera.AutoFocusCallback() {
          ...
          public void onAutoFocus(boolean success, Camera camera) {
            ...
            // in theory we should only ever get one call to onAutoFocus(), but some Samsung phones
            // at least can call the callback multiple times
            // see http://stackoverflow.com/questions/36316195/take-picture-fails-on-samsung-phones
            // needed to fix problem on Samsung S7 with flash auto/on and continuous picture focus
            // where it would claim failed to take picture even though it'd succeeded,
            // because we repeatedly call takePicture(), and the subsequent ones cause a runtime
            // exception
            if (!done_autofocus) {
              done_autofocus = true;
  ...
  public void cancelAutoFocus() {
    try {
      ...
      // https://sourceforge.net/p/opencamera/tickets/4/ made on 20140520
  ...
  public void setContinuousFocusMoveCallback(final ContinuousFocusMoveCallback cb) {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
      ...
      try {
          ...
          camera.setAutoFocusMoveCallback(
              new AutoFocusMoveCallback() {
                ...
                public void onAutoFocusMoving(boolean start, Camera camera) {
                  if (MyDebug.LOG) Log.d(TAG, "onAutoFocusMoving: " + start);
        ...
        // received RuntimeException reports from some users on Google Play - seems to be older
        // devices, but still important to catch!
  ...
  private static class TakePictureShutterCallback implements Camera.ShutterCallback {
    // don't do anything here, but we need to implement the callback to get the shutter sound (at
    // least on Galaxy Nexus and Nexus 7)
    @Override
  ...
  private void takePictureNow(
      final CameraController.PictureCallback picture, final ErrorCallback error) {
    if (MyDebug.LOG) Log.d(TAG, "takePictureNow");

    final Camera.ShutterCallback shutter = new TakePictureShutterCallback();
    final Camera.PictureCallback camera_jpeg =
        picture == null
            ? null
              ...
              public void onPictureTaken(byte[] data, Camera cam) {
                ...
                if (want_expo_bracketing && n_burst > 1) {
                  ...
                  if (pending_burst_images.size()
                    ...
                    // need to start preview again: otherwise fail to take subsequent photos on
                    // Nexus 6
                    // and Nexus 7; on Galaxy Nexus we succeed, but exposure compensation has no
                    // effect
                    try {
                      startPreview();
                    ...
                    handler.postDelayed(
                        new Runnable() {
                          @Override
                          public void run() {
                            if (MyDebug.LOG) Log.d(TAG, "take picture after delay for next expo");
                            if (camera
                                != null) { // make sure camera wasn't released in the meantime
    ...
    try {
      ...
      // just in case? We got a RuntimeException report here from 1 user on Google Play; I also
      // encountered it myself once of Galaxy Nexus when starting up
  ...
  public void takePicture(
    ...
    if (want_expo_bracketing) {
      ...
      int steps =
          (int)
              ((stops_per_image + 1.0e-5)
                  / exposure_step); // need to add a small amount, otherwise we can round down
      ...
      // if we change the order, remember to update the code that re-orders for passing resultant
      // images back to picture.onBurstPictureTaken()
    ...
    if (frontscreen_flash) {
      ...
      // take picture after a delay, to allow autoexposure and autofocus to update (unlike
      // CameraController2, we can't tell when this happens, so we just wait for a fixed delay)
      Handler handler = new Handler();
      handler.postDelayed(
          new Runnable() {
            @Override
  ...
  public void unlock() {
    this.stopPreview(); // although not documented, we need to stop preview to prevent device freeze
    // or video errors shortly after video recording starts on some devices
    // (e.g., device freeze on Samsung Galaxy S2 - I could reproduce this on
    // Samsung RTL; also video recording fails and preview becomes corrupted on
    // Galaxy S3 variant "SGH-I747-US2"); also see
