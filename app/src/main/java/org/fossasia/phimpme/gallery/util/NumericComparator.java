...
public class NumericComparator {
  ...
  public static int filevercmp(String s1, String s2) {
    ...
    if (0 == strcmp(".", s1)) return -1;
    if (0 == strcmp(".", s2)) return 1;
    if (0 == strcmp("..", s1)) return -1;
    if (0 == strcmp("..", s2)) return 1;

    /* special handle for other hidden files */
    if (s1.codePointAt(0) == '.' && s2.codePointAt(0) != '.') return -1;
    if (s1.codePointAt(0) != '.' && s2.codePointAt(0) == '.') return 1;
    ...
    if ((s1_suffix.length() > 0 || s2_suffix.length() > 0)
        && (s1_len == s2_len)
  ...
  public static int verrevcmp(String s1, String s2) {
    ...
    while (s1_pos < s1.length() || s2_pos < s2.length()) {
      ...
      while (s1_pos < s1.length()
          && s2_pos < s2.length()
          && c_isdigit(s1.codePointAt(s1_pos))
          && c_isdigit(s2.codePointAt(s2_pos))) {
        if (first_diff == 0) first_diff = s1.codePointAt(s1_pos) - s2.codePointAt(s2_pos);
        s1_pos++;
        s2_pos++;
      }
      if (s1_pos < s1.length() && c_isdigit(s1.codePointAt(s1_pos))) return 1;
      if (s2_pos < s2.length() && c_isdigit(s2.codePointAt(s2_pos))) return -1;
