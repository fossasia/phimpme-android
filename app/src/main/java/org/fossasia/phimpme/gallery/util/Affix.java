...
public class Affix {
  ...
  public static void AffixBitmapList(Context ctx, ArrayList<Bitmap> bitmapArray, Options options) {
    ...
    if (options.isVertical())
      unionBitmap =
          Bitmap.createBitmap(
              getMaxBitmapWidth(bitmapArray),
              getBitmapsHeight(bitmapArray),
              Bitmap.Config.ARGB_8888);
    else
      unionBitmap =
          Bitmap.createBitmap(
              getBitmapsWidth(bitmapArray),
              getMaxBitmapHeight(bitmapArray),
              Bitmap.Config.ARGB_8888);
  ...
  private static void saveFile(Context context, Bitmap bmp, Options options) {
    try {
      File file =
          new File(
              options.getFolderPath(),
              System.currentTimeMillis() + "." + options.getExtensionFormat());
  ...
  private static int getMaxBitmapWidth(ArrayList<Bitmap> bpmHeightArray) {
    ...
    for (int i = 1; i < bpmHeightArray.size(); i++) {
      if (width < bpmHeightArray.get(i).getWidth()) width = bpmHeightArray.get(i).getWidth();
    }
    return width;
  }
  ...
  private static int getMaxBitmapHeight(ArrayList<Bitmap> bpmHeightArray) {
    ...
    for (int i = 1; i < bpmHeightArray.size(); i++) {
      if (height < bpmHeightArray.get(i).getHeight()) height = bpmHeightArray.get(i).getHeight();
    }
    return height;
  }
