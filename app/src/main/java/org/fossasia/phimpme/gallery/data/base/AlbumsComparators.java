...
public class AlbumsComparators {
  ...
  public static Comparator<Album> getComparator(
      SortingMode sortingMode, SortingOrder sortingOrder) {
    switch (sortingMode) {
      case NAME:
  ...
  private static Comparator<Album> getSizeComparator(final SortingOrder sortingOrder) {
    return new Comparator<Album>() {
      public int compare(Album a1, Album a2) {
        switch (sortingOrder) {
          case ASCENDING:
            if (a1.isPinned() == a2.isPinned()) return a1.getCount() - a2.getCount();
            return a1.isPinned() ? -1 : 1;

          case DESCENDING:
          default:
            if (a1.isPinned() == a2.isPinned()) return a2.getCount() - a1.getCount();
            return a2.isPinned() ? 1 : -1;
        }
      }
    };
    ...
    return new Comparator<Album>() {
      public int compare(Album a1, Album a2) {
        switch (sortingOrder) {
          case ASCENDING:
            if (a1.isPinned() == a2.isPinned())
              return NumericComparator.filevercmp(
                  a1.getName().toLowerCase(), a2.getName().toLowerCase());
            return a1.isPinned() ? -1 : 1;
          ...
          default:
            if (a1.isPinned() == a2.isPinned())
              return NumericComparator.filevercmp(
                  a2.getName().toLowerCase(), a1.getName().toLowerCase());
            return a2.isPinned() ? 1 : -1;
        }
      }
    };
