...
public class Album implements Serializable {
  ...
  public void setCurrentPhotoIndex(Media m) {
    setCurrentPhotoIndex(media.indexOf(m));
  }

  public Media getCurrentMedia() {
    return getMedia(currentMediaIndex);
  }
  ...
  public Media getCoverAlbum() {
    if (hasCustomCover()) return new Media(settings.getCoverPath());
    if (media.size() > 0) return media.get(0);
    return new Media();
  }
  ...
  public ArrayList<Media> getMedia() {
    ...
    switch (getFilterMode()) {
      case ALL:
        ...
        break;
      case GIF:
        for (Media media1 : media) if (media1.isGif()) mediaArrayList.add(media1);
        break;
      case IMAGES:
        for (Media media1 : media) if (media1.isImage()) mediaArrayList.add(media1);
        break;
      default:
        break;
    }
  ...
  private ArrayList<Media> getMedia(Context context) {
    ...
    // TODO: 18/08/16
    if (isFromMediaStore()) {
      mediaArrayList.addAll(MediaStoreProvider.getMedia(context, id));
    } else {
      mediaArrayList.addAll(
          StorageProvider.getMedia(getPath(), SP.getBoolean("set_include_video", false)));
    }
    return mediaArrayList;
  }
    ...
    try {
      ...
      if (success = moveMedia(context, from, targetDir)) {
        scanFile(
            context,
            new String[] {
              from, StringUtils.getPhotoPathMoved(getCurrentMedia().getPath(), targetDir)
            });
        media.remove(getCurrentMediaIndex());
        if (getPreviewPath() != null && from.equals(getPreviewPath())) removeCoverAlbum(context);
        setCount(media.size());
      }
    } catch (Exception e) {
      e.printStackTrace();
        ...
        for (int i = 0; i < selectedMedias.size(); i++) {
          ...
          if (moveMedia(context, selectedMedias.get(i).getPath(), targetDir)) {
            ...
            scanFile(
                context,
                new String[] {
                  from, StringUtils.getPhotoPathMoved(selectedMedias.get(i).getPath(), targetDir)
                },
                new MediaScannerConnection.OnScanCompletedListener() {
                  @Override
                  public void onScanCompleted(String s, Uri uri) {
  ...
  /**
   * On longpress, it finds the last or the first selected image before or after the targetIndex and
   * selects them all.
   *
   * @param targetIndex
   * @param adapter
  ...
  public int getIndex(Media m) {
    return media.indexOf(m);
  }

  public void clearSelectedPhotos() {
    for (Media m : media) m.setSelected(false);
  ...
  public void sortPhotos() {
    Collections.sort(
        media,
        MediaComparators.getComparator(settings.getSortingMode(), settings.getSortingOrder()));
  }

  public boolean copySelectedPhotos(Context context, String folderPath) {
    boolean success = true;
    for (Media media : selectedMedias)
      if (!copyPhoto(context, media.getPath(), folderPath)) success = false;
    return success;
  }
  ...
  public boolean copyPhoto(Context context, String olderPath, String folderPath) {
    ...
    try {
      ...
      if (success = ContentHelper.copyFile(context, from, to))
        scanFile(
            context,
            new String[] {StringUtils.getPhotoPathMoved(getCurrentMedia().getPath(), folderPath)});
  ...
  public boolean deleteSelectedMedia(Context context) {
    ...
    for (Media selectedMedia : selectedMedias) {
      if (deleteMedia(context, selectedMedia)) media.remove(selectedMedia);
      else success = false;
