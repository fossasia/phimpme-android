...
public class AlertDialogsHelper {
  ...
  public static AlertDialog getInsertTextDialog(
      final ThemedActivity activity,
  ...
  public static AlertDialog getTextDialog(
      final ThemedActivity activity,
      AlertDialog.Builder textDialogBuilder,
      @StringRes int title,
    ...
    ((CardView) dialogLayout.findViewById(R.id.message_card))
        .setCardBackgroundColor(activity.getCardBackgroundColor());
  ...
  public static AlertDialog getTextCheckboxDialog(
      final ThemedActivity activity,
      AlertDialog.Builder textDialogBuilder,
      @StringRes int title,
      @StringRes int Message,
  ...
  public static AlertDialog getProgressDialog(
    ...
    dialogTitle.setBackgroundColor(activity.getPrimaryColor());
    ((CardView) dialogLayout.findViewById(R.id.progress_dialog_card))
        .setCardBackgroundColor(activity.getCardBackgroundColor());
  ...
  public static AlertDialog getDetailsDialog(
      final ThemedActivity activity, AlertDialog.Builder detailsDialogBuilder, final Media f) {
    MediaDetailsMap<String, String> mainDetails =
        f.getMainDetails(activity.getApplicationContext());
    final View dialogLayout =
        activity.getLayoutInflater().inflate(R.layout.dialog_media_detail, null);
    ...
    ((CardView) dialogLayout.findViewById(R.id.photo_details_card))
        .setCardBackgroundColor(activity.getCardBackgroundColor());
    ...
    if ((location = f.getGeoLocation()) != null) {
      ...
      StaticMapProvider staticMapProvider =
          StaticMapProvider.fromValue(
              SP.getInt(
                  activity.getString(R.string.preference_map_provider),
  ...
  public static AlertDialog getAlbumDetailsDialog(
      final ThemedActivity activity, AlertDialog.Builder detailsDialogBuilder, final Album f) {
    MediaDetailsMap<String, String> mainDetails =
        f.getAlbumDetails(activity.getApplicationContext());
    final View dialogLayout =
        activity.getLayoutInflater().inflate(R.layout.dialog_album_detail, null);
    dialogLayout
        .findViewById(R.id.album_details_title)
        .setBackgroundColor(activity.getPrimaryColor());
    ((CardView) dialogLayout.findViewById(R.id.album_details_card))
        .setCardBackgroundColor(activity.getCardBackgroundColor());
    detailsDialogBuilder.setView(dialogLayout);
    loadDetails(dialogLayout, activity, mainDetails);
    return detailsDialogBuilder.create();
  ...
  private static void loadDetails(
      View dialogLayout, ThemedActivity activity, MediaDetailsMap<String, String> metadata) {
    int textColor =
        activity.getBaseTheme() != ThemeHelper.LIGHT_THEME
            ? Color.parseColor("#FAFAFA")
            : Color.parseColor("#2b2b2b");
    ((ImageView) dialogLayout.findViewById(R.id.icon_folder))
        .setColorFilter(activity.getAccentColor());
  ...
  public static void setButtonTextColor(int[] buttons, int color, AlertDialog alertDialog) {
    for (int button : buttons) alertDialog.getButton(button).setTextColor(color);
  }
}
