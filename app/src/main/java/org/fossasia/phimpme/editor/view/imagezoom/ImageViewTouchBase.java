...
import org.fossasia.phimpme.editor.view.imagezoom.utils.IDisposable;
...
public abstract class ImageViewTouchBase extends android.support.v7.widget.AppCompatImageView
   ...
   * @param min_zoom the optional minimum scale, pass {@link #ZOOM_INVALID} to use the default
   *     min_zoom
   * @param max_zoom the optional maximum scale, pass {@link #ZOOM_INVALID} to use the default
   *     max_zoom
   */
  ...
  protected void _setImageDrawable(
    ...
    if (drawable != null) {
      ...
      if (LOG_ENABLED) {
        Log.d(
            LOG_TAG, "size: " + drawable.getIntrinsicWidth() + "x" + drawable.getIntrinsicHeight());
      }
   ...
   * This matrix can be used in the next call to the {@link #setImageDrawable(Drawable, Matrix,
   * float, float)} to restore the same view state of the previous {@link Bitmap}.<br>
   * Example:
   *
   * <pre>
  ...
  public void printMatrix(Matrix matrix) {
    ...
    Log.d(
        LOG_TAG,
        "matrix: { x: " + tx + ", y: " + ty + ", scalex: " + scalex + ", scaley: " + scaley + " }");
  }
