...
5. *We use danger, so update goes to https://travis-ci.org and login with GitHub. Next, you will see a `+` near my repositories. Click on it and select your fork enabling it for Travis. Then go the dashboard and add two environment variables which two won't get displayed in the terminal(There's an option). The first one must be named `USERNAME` and should contain your GitHub username. The second one should be named `API_TOKEN` and should contain a GitHub personal token with at least scope to the public repository. It enables fast and easy moderation adequate testing.*
...
permanently any contributor to other behaviors that they deem inappropriate,
